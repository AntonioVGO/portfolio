            ...
            <div className="flex flex-row justify-center gap-x-2">
                {
                    data?.forecast.forecastday.map((item) => (
                        <div className="bg-gray-200 dark:bg-white rounded-md py-2 px-2 w-1/4 h-auto flex flex-col">
                            <p className="text-xs text-center">{item.date}</p>
                            <p className="text-xl text-center">{Math.round(item.day.avgtemp_c)}°</p>
                            <div className="w-full flex flex-col justify-center items-center">
                                <img src={data?.current.condition.icon} className="w-13"></img>
                            </div>
                            <p className="text-xs text-center">Max: {Math.round(item.day.mintemp_c)}°</p>
                            <p className="text-xs text-center">Min: {Math.round(item.day.maxtemp_c)}°</p>
                        </div>
                    ))
                }
            </div>
