...
export const App = () => {
  ...
  const {data, hasError, isFetching, fetchData} = useFetch<Weather>({url:`http://api.weatherapi.com/v1/current.json?key=c56bb0dd602c49438ba82122231307&q=${location}&aqi=no`})

  return (
    <>
      <div className="w-96 h-96 bg-cyan-500 m-20 rounded-xl flex flex-col">

        <div className='flex'>
          <div className="relative left-4 py-2">
            <img src={data?.current.condition.icon} className="w-28"></img>
            <p className="text-7xl">{data?.current.temp_c}°</p>
          </div>
        </div>
      </div>
    </>
